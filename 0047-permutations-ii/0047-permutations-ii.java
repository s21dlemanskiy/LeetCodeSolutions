class Solution {
    ...
    static List<List<Integer>> result; // список масивов, результат функции permuteUnique
    ...
    public static void recursiveProduct(){  // рекурсивная функция нахождения декартого произведения
        ...
        mainloop: for (int i = 0; i < used.length; i++) {  // если в used еще есть -1 нужно их рекурсивно заполнять
            for (int j = 0; j < i; j++) {
                if (used[j] == -1 && nums_pub[i] == nums_pub[j]){
                    continue mainloop;
                }
            }
            if (used[i] == -1){
